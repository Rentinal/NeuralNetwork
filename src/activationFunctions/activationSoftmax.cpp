...
void activationSoftmax::backward(const dMatrix &dValues)
{
  ...
  for (uint32_t i = 0; i < m_output.numRows(); i++) {
    ...
    for (int32_t j = 0; j < static_cast<int32_t>(outputVector.numCols()); j++) {
      m_dInput.put(static_cast<int32_t>(i), j, outputVector.at(0, j));
    }
  }
}
